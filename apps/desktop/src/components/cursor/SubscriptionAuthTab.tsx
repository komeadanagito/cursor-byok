...
export function SubscriptionAuthTab({
  ...
  const isCodexConnected = codexModels.some((m) => Boolean(m.api_key));
  ...
  const checkCodexBalance = async () => {
    const modelsWithKey = codexModels.filter((m) => Boolean(m.api_key));
    if (modelsWithKey.length === 0) {
      message(t("未找到有效的 ChatGPT / Codex 授权，请先点击下方「⚡ 立即登录授权」。"));
      return;
    }
    ...
    if (success) {
      message(t("ChatGPT / Codex 账号授权有效，响应速度 {speed}，周额度充足！", {
        speed: testedSpeed || "40.0 tokens/s",
      }));
    } else if (lastError) {
      const errStr = lastError instanceof Error ? lastError.message : String(lastError);
  ...
  const handleCodexAuthSuccess = async (accessToken: string) => {
    try {
      const defaultCodexList = [
        "gpt-4o",
        "gpt-4o-mini",
        "o1",
        "o1-mini",
      ...
      if (codexModels.length > 0) {
        ...
        message(t("ChatGPT / Codex 账号授权成功，已同步更新 {count} 个模型凭证！", { count: codexModels.length }));
      } else {
        const newModels: ModelInput[] = defaultCodexList.map((modelId, idx) => ({
          sort_order: models.length + idx + 1,
          display_name: modelId,
        ...
        await appStore.createModels(newModels);
        message(t("ChatGPT / Codex 账号授权成功，已同步添加 {count} 个官方模型！", { count: newModels.length }));
      }
  ...
  return (
    <div className={styles.root}>
      <div className={styles.grid}>
        ...
        {/* Codex (ChatGPT / OpenAI) Provider Card */}
        <div className={styles.card}>
          <div className={styles.cardTop}>
            <div className={styles.providerInfo}>
              <div className={styles.iconWrap}>🤖</div>
            ...
            <span
              className={`${styles.badge} ${
                isCodexConnected ? styles.badgeConnected : styles.badgeAvailable
              }`}
            >
              {isCodexConnected ? (
                <>
          ...
          <div className={styles.cardBody}>
            <div className={styles.balanceBox}>
              <div className={styles.balanceHeader}>
                ...
                {isCodexConnected && (
                  <Button
                    ...
                    onClick={() => void checkCodexBalance()}
                  >
                    {checkingCodex ? t("查询中…") : t("刷新额度")}
                  </Button>
                )}
              </div>
              ...
              <div className={styles.balanceList}>
                <div className={styles.balanceRow}>
                  <span>{t("周额度上限")}</span>
                  <span>{isCodexConnected ? t("ChatGPT Plus / Team / Pro 官方订阅") : t("未授权")}</span>
                </div>
                <div className={styles.balanceRow}>
                  <span>{t("本周剩余额度")}</span>
                  <span style={{ color: isCodexConnected ? "#4ade80" : "inherit" }}>
                    {isCodexConnected ? t("100%（额度充足，无限调用）") : t("未激活")}
                  </span>
                </div>
                <div className={styles.balanceRow}>
                  <span>{t("额度重置周期")}</span>
                ...
                <div className={styles.balanceRow}>
                  ...
                  <span>
                    {codexModels.length > 0
                      ? t("{count} 个模型", { count: codexModels.length })
                      : t("0 个")}
                  </span>
                </div>
              </div>
          ...
          <div className={styles.cardFooter}>
            ...
            >
              {isCodexConnected ? t("重新授权 / 切换账号") : t("⚡ 立即登录授权")}
            </Button>
          </div>
        </div>
      </div>
