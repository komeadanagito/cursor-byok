...
type EmbeddedGrokAuthProps = {
  onSuccess: (accessToken: string) => void;
};

export function EmbeddedGrokAuth({ onSuccess }: EmbeddedGrokAuthProps) {
  ...
  // Polling loop
  useEffect(() => {
    if (pollStatus !== "polling" || !deviceData) return;
  ...
  return (
    <div className={styles.modalContent}>
      <div className={styles.banner}>
        <span>
          {t("使用 X Premium+ 或 SuperGrok 订阅账号登录，直接使用 Grok 官方模型额度，无需单独购买 API Key。")}
        </span>
      </div>
